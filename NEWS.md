...
* A run's per-event `debugMessage()` no longer errors "'list' object cannot be coerced to type 'double'" when `debug` is a list. The event-loop call `debugMessage(ifelse(debug < 1, debug + 1, debug), ...)` was unguarded, unlike the equivalent in `spades.R` (`if (!is.numeric(debug)) debug else ifelse(...)`), so a stage WITH events crashed under a list `debug` even after the `setPaths` fix above. Guarded it the same way; the #322 regression test now also runs a sim with events under a list `debug`. (#322)
