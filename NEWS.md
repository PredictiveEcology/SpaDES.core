...
* parsing of modules is now more intelligent, allowing for functions (the current norm) and also objects that are defined using the sim object. These objects can, for example, be used to help define parameters, for example, e.g., `startSimPlus1 <- start(sim) + 1` can be defined in the module and used in `defineModule`
