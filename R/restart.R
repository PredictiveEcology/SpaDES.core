...
#' there will be a list in the `simList` called `.recoverableObjs`. These record the elements of simList that have 
#' changed over a number of events equal to the number chosen for `options('spades.recoveryMode')`.
...
#' trajectory.
#'
#' @note The `simList` will be in the state it was `numEvents` prior to the event that led to the error.
#'
...
#'
#' @param numEvents Numeric. Default is Inf (i.e., all available). The number of events
#'   to be rewound. In the `simList`, if `options('spades.recoveryMode')` is set to `TRUE` or a numeric, then
...
#'   the event that is `numEvents` prior from the first event in `events(sim)`. 
#'
