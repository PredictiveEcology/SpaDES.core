...
test_that("3 levels of parent and child modules load and show correctly", {
  ...
  if (Sys.which("glpsol") == "") {
    ...
    ## On Windows CI, GLPK is bundled and the test runs, but
    ## igraph::cluster_optimal() returns a different number of communities
    ## here than on Linux/macOS for this 3-level hierarchy. Algorithm
    ## sensitivity to igraph version, not a SpaDES.core bug.
    skip("igraph cluster_optimal community count differs on Windows")
  } else {
