...
Note that if there were any visualizations (here we turned them off with `.plots = ""` above) they will happen the first time through, but not the cached times.
...
system.time({
  randomSim <- spades(Copy(mySim), .plots = "",
                      notOlderThan = Sys.time(), debug = TRUE)
})

# faster the second time
system.time({
  randomSimCached <- spades(Copy(mySim), .plots = "", debug = TRUE)
})
```
