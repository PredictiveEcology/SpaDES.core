...
As part of a reproducible work flow, caching of various function calls is a critical component.
...
Important to this is dealing correctly with the `simList`, which is an object that has slot that is an environment. 
...
`Cache` will use the `cachePath` argument from a call to `cachePath(sim)`, taking the `sim` from the call stack.
Similarly, if no `cachePath` argument is specified, then it will use `getOption("reproducible.cachePath")`.
...
system.time({
  randomSim <- spades(Copy(mySim), .plots = NA,
                      notOlderThan = Sys.time(), debug = TRUE)
})

# faster the second time
system.time({
  randomSimCached <- spades(Copy(mySim), .plots = NA, debug = TRUE)
})
```
...
Function-level caching is provided by `reproducible::Cache()` and is
documented there; see `?reproducible::Cache` for the full set of arguments
and behaviour. Any call can be wrapped in `Cache()` so that re-running it
