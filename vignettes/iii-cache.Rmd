...
Similarly, if no `cachePath` argument is specified, then it will use `getOption("reproducible.cachePath")`, which will, by default, be a temporary location with no persistence between R sessions!
To persist between sessions, use `SpaDES.core::setPaths()` every session.
